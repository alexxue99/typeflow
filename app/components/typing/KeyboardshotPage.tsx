  ...
  return <section className="type-page" data-hide-cursor={settings.hideCursorDuringTests && status === "active" && !cursorShownByMouse ? "true" : "false"} onMouseMove={() => {
    ...
    <div ref={panelRef} className="typing-panel keyboardshot-panel" data-status={status} data-glow-fade={settings.keyboardshotGlowFade ? "on" : "off"} tabIndex={0} onKeyDown={onKeyDown} onClick={() => panelRef.current?.focus()} aria-label="Keyboardshot game">
     {/* <p className="keyboardshot-instructions"> {status === "idle" ? "Press any highlighted key to start. Each hit immediately reveals a new target." : ""}</p> */}
      ...
      {status !== "done" && (
        <p className="keyboardshot-status">{status === "idle" ? `Click anywhere here, then press any highlighted key to start. Press ${settings.resetHotkey} to reset.` : feedback === "hit" ? "Hit!" : feedback === "miss" ? "Miss — find a highlighted key" : ""}</p>
      )}
      {status === "done" && <p className="keyboardshot-status typing-reset-prompt">Test finished! Press {settings.resetHotkey} to reset.</p>}
    </div>
    {status === "done" && <div className="result-card"><div className="result-card-summary"><div><span className="eyebrow">Nice reflexes</span><h2>{settings.sessionType === "words" ? (elapsedMilliseconds / 1000).toFixed(3) + "s" : (2 * hits - attempts) + " point" + (2 * hits - attempts !== 1 ? "s" : "") + " (" + hits + " hit" + (hits !== 1 ? "s" : "") + ", " + (attempts - hits) + " miss" + (attempts - hits !== 1 ? "es" : "") + ")"} · {(hits / elapsedMilliseconds * 1000).toFixed(2) + " hits/s"} · {accuracy}% accuracy</h2></div><div className="session-actions"><button className="icon-button" onClick={() => restart()} aria-label="Restart session">↻</button></div></div></div>}
