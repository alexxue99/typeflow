...
export function ModeSettings({ mode, settings, setSettings, onRestart }: Props) {
  ...
  const canResetTestSettings = mode === "flow" || mode === "zen" || mode === "cadence" || mode === "keyboardshot";
  const sequenceMode = mode === "flow" || mode === "zen" || mode === "cadence";
  let countName = "Word count";
  let countSetting = "Words";
  if (mode === "keyboardshot") {countName = "Target count"; countSetting = "Targets";}
  if (mode === "workout" || mode === "zen" || mode === "cadence") {countName = "Block count"; countSetting = "Blocks";}
  return <details className="mode-settings">
    <summary>Mode settings</summary>
    <div className="mode-settings-grid">
      <label>Session type<StableSelect value={settings.sessionType} onChange={(e) => update("sessionType", e.target.value as Settings["sessionType"])}><option value="time">Time</option><option value="words">{countSetting}</option><option value="endless">Endless</option></StableSelect></label>
      {settings.sessionType === "time" && <label>Timer duration<StableSelect value={settings.duration} onChange={(e) => update("duration", Number(e.target.value))}>{[15, 30, 60, 120].map((n) => <option key={n} value={n}>{n} seconds</option>)}</StableSelect></label>}
      ...
      {mode === "cadence" && <label>Delay <strong>{settings.cadenceDelay.toFixed(1)}s</strong><input type="range" min="0.1" max="0.5" step="0.1" value={settings.cadenceDelay} onChange={(e) => update("cadenceDelay", Number(e.target.value))} /></label>}
      {mode === "cadence" && <label className="check-row"><input type="checkbox" checked={settings.cadenceBlockSound ?? false} onChange={(e) => update("cadenceBlockSound", e.target.checked)} /> Play sound when next block is available</label>}
      ...
      {(mode === "zen" || mode === "cadence" || mode === "keyboardshot") && <label className="check-row"><input type="checkbox" checked={settings.useStandardLetterFrequency} onChange={(e) => update("useStandardLetterFrequency", e.target.checked)} /> Use standard letter frequency</label>}
      {(mode === "zen" || mode === "cadence") && <label>Block size <strong>{settings.zenBlockSize}</strong><input type="range" min="1" max="10" value={settings.zenBlockSize} onChange={(e) => update("zenBlockSize", Number(e.target.value))} /></label>}
